...
from tron1_mjx_env_basic import Tron1MjxBasicEnv
...
CONFIG = TrainConfig(
    n_envs=_env_int("MJX_N_ENVS", 16),
    total_timesteps=_env_int("MJX_TOTAL_TIMESTEPS", 1_000_000),
    rollout_len=_env_int("MJX_ROLLOUT_LEN", 2048),
    update_epochs=_env_int("MJX_UPDATE_EPOCHS", 10),
    num_minibatches=_env_int("MJX_NUM_MINIBATCHES", 256),
    learning_rate=_env_float("MJX_LEARNING_RATE", 5e-4),
    clip_coef=_env_float("MJX_CLIP_COEF", 0.25),
    ent_coef=_env_float("MJX_ENT_COEF", 0.0),
    vf_coef=_env_float("MJX_VF_COEF", 0.5),
    max_grad_norm=_env_float("MJX_MAX_GRAD_NORM", 0.5),
    init_log_std=_env_float("MJX_INIT_LOG_STD", 0.0),
    checkpoint_every=_env_int("MJX_CHECKPOINT_EVERY", 100_000),
)
SAVE_ENABLED = _env_bool("MJX_SAVE", True)
RUN_NAME = os.environ.get("MJX_RUN_NAME", "basic_v2")
